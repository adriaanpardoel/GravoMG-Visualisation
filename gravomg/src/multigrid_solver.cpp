...
namespace GravoMG {
	...
	void MultigridSolver::constructProlongation()
	{
		...
		while (levelPoints.rows() > lowBound && k < 10) {
            ...
            if (debug) neighHierarchy.push_back(neighLevelK);
			...
			int edgesfound = 0;
			int fallbackCount = 0;
			if (debug) prolongationFallback.push_back(std::vector<bool>(DoF[k]));
				...
				if (neighborsList[coarseIdx].empty()) {
					...
					if (triFound) {
						...
						if (edgeFound) {
							...
							++fallbackCount;
                            if (debug) prolongationFallback[k][fineIdx] = true;
						}
						++notrisfound;
					}
				}
